...
class Transformer:
    ...
    def remove_short_fragments(predicted_midi, min_length=10):
        batch_size, notes, time_frames = predicted_midi.shape
        for batch in range(batch_size):
            for note in range(notes):
                segment_start = None
                for time_frame in range(time_frames):
                    if segment_start is not None:
                        if predicted_midi[batch, note, time_frame] == 0:
                            length = time_frame - segment_start
                            if length < min_length:
                                predicted_midi[batch, note, segment_start:time_frame] = 0
                            segment_start = None
                    else:
                        segment_start = time_frame if predicted_midi[batch, note, time_frame] == 1 else segment_start
            ...
            for note in range(notes):
                ...
                for time_frame in range(time_frames):
                    if prev_segment_end is not None:
                        if predicted_midi[batch, note, time_frame] == 1:
                            length = time_frame - prev_segment_end
                            if length < min_length:
                                predicted_midi[batch, note, prev_segment_end:time_frame] = 1
