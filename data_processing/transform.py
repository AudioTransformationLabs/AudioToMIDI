...
def split_audio_midi_pair(
    ...
    spectrogram = transform_audio(audio_path, transform)
    num_time_frames = spectrogram.shape[-1]

    piano_roll = transform_midi(midi_path, num_time_frames)

    audio_chunks = split_into_chunks(
        spectrogram, chunk_length, hop_length
    )
    midi_chunks = split_into_chunks(
...
if __name__ == "__main__":
    ...
    spectrogram = transform_audio(audio_path, mfcc_transform())
    print(spectrogram.shape)

    spectrogram = transform_audio(audio_path, mel_spec_transform())
    print(spectrogram.shape)